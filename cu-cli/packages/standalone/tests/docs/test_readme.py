...
def _az(*args, **kwargs):
    return invoke_azure(list(args), **kwargs)
...
def test_macos_executable_name():
    ...
    result = _run("--help", executable="cu-cli")
    # endregion
...
def test_frontends_share_profile_settings(monkeypatch, sample_invoice):
    ...
    _run("profile", "set", "api_key", "playback-dummy-key")
...
def test_inline_analysis_with_preview_api(cloud_profile, sample_invoice):
    with use_cassette("analyze_inline_preview"):
        ...
        result = _run(
            ...
            "sample_invoice.pdf", "--analyzer", "prebuilt-layout", "--json",
        )
        # endregion
...
def test_analyze_with_prebuilt_layout(cloud_profile, sample_invoice):
    ...
    with use_cassette("analyze_single"):
        ...
        short = _run(
            "analyze", "sample_invoice.pdf", "-a", "prebuilt-layout",
            comment="`-a` is the short form of `--analyzer`.",
        )
        # endregion
...
def test_analyze_remote_url_keeps_sas_private(cloud_profile, monkeypatch):
    ...
    result = _run(
        "analyze", "--url",
        "https://storage.example.net/container/video.mp4?sv=<version>&sp=r&sig=<signature>",
...
def test_analyze_invoice_fields(cloud_profile, sample_invoice):
    with use_cassette("analyze_prebuilt_invoice_json"):
        ...
        result = _run("analyze", "sample_invoice.pdf", "-a", "prebuilt-invoice", "--json")
        # endregion
...
def test_create_custom_analyzer(cloud_profile, sample_invoice, monkeypatch):
    ...
    result = _run("analyze", "sample_invoice.pdf", "-a", "invoice_v1", "--json")
    # endregion
