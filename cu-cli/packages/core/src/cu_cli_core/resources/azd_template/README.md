...
Generated projects using the version 1 postprovision contract are supported by
all `0.1.x` CLI releases. The legacy hidden model-setup command remains available
throughout `0.1.x` for projects generated by earlier beta builds. Regenerate the
project before upgrading to a later minor release if its release notes do not
explicitly retain that contract.
